...
def visualize_data(data, month=None):
    ...
    if month is None:
        ...
        filtered_data = data[(data['hourly_timestamp'].dt.year == year) & 
    ...
    daily_data = filtered_data.groupby('daily_timestamp', as_index=False).agg({'query_count': 'sum'})

    # --- Hourly Query Count Plot ---
    plt.figure(figsize=(14, 6))
    plt.plot(filtered_data['hourly_timestamp'], filtered_data['query_count'], label="Hourly Query Count", color="blue", linestyle="-")
    plt.axvline(x=filtered_data['hourly_timestamp'].max() - pd.Timedelta(days=7), color="red", linestyle="--", label="Train-Test Split")
    plt.xlabel("Time (Hourly)")
    plt.ylabel("Query Count (Normalized)")
    plt.title(f"Hourly Query Count Series for {year}-{month if month else 'Full Dataset'}")
    ...
    plt.figure(figsize=(14, 6))
    plt.plot(daily_data['daily_timestamp'], daily_data['query_count'], label="Daily Query Count", marker="o", linestyle="-", color="blue")
    plt.axvline(x=daily_data['daily_timestamp'].max() - pd.Timedelta(days=7), color="red", linestyle="--", label="Train-Test Split")
    plt.xlabel("Time (Daily)")
    plt.ylabel("Query Count (Normalized)")
    plt.title(f"Daily Query Count Series for {year}-{month if month else 'Full Dataset'}")
    plt.xticks(rotation=45)
    plt.grid(True, linestyle="--", alpha=0.6)
    plt.legend()
    plt.show()
...
def visualize_prediction(training_data, test_data, prediction):
    ...
    plt.plot(training_data['hourly_timestamp'], training_data['query_count'], label="Training Data", color="gray", linestyle="-", alpha=0.6)
    plt.plot(test_data['hourly_timestamp'], test_data['query_count'], label="Actual Test Data", color="blue", linestyle="-")
    plt.plot(prediction['hourly_timestamp'], prediction['query_count'], label="Predicted Data", color="red", linestyle="dashed")
    
    plt.axvline(x=test_data['hourly_timestamp'].min(), color="black", linestyle="--", label="Train-Test Split")
